...
from base.admin_mixins import (
    AppAdmin,
    HideAuditFieldsMixin,
    UnscopedAdminMixin,
)
from . import models
...
class CompanyAdmin(AppAdmin):
    ...
    fieldsets = (
        ...
        (
            "Identity (from Partner / read-only)",
            {
                ...
                "description": (
                    "These fields mirror the linked Partner. "
                    "Edit them on the Partner record."
                ),
            },
        ),
    )
    ...
    def company_email(self, obj):
        return getattr(getattr(obj.partner, "email", None), "", "") if obj.partner_id else ""
    company_email.short_description = "Email"

    def company_phone(self, obj):
        return getattr(getattr(obj.partner, "phone", None), "", "") if obj.partner_id else ""
    company_phone.short_description = "Phone"

    def company_website(self, obj):
        return getattr(getattr(obj.partner, "website", None), "", "") if obj.partner_id else ""
    company_website.short_description = "Website"

    def company_vat(self, obj):
        return getattr(getattr(obj.partner, "vat", None), "", "") if obj.partner_id else ""
    company_vat.short_description = "VAT"

    def company_registry(self, obj):
        return getattr(getattr(obj.partner, "company_registry", None), "", "") if obj.partner_id else ""
...
class UserAdmin(UnscopedAdminMixin, HideAuditFieldsMixin, DjangoUserAdmin):
    ...
    def save_related(self, request, form, formsets, change):
        ...
        if (
            ...
            and not user.companies.filter(pk=user.company_id).exists()
        ):
            pass
        return result
